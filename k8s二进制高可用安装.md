...
```bash
yum install wget jq psmisc vim net-tools telnet yum-utils device-mapper-persistent-data lvm2 git ipvsadm ipset sysstat conntrack libseccomp chrony bash-completion lrzsz htop socat rsyslog tar zip unzip -y
```
...
#### 1.3 发送k8s二进制文件到其他master节点

```bash
MasterNodes='k8s-master02 k8s-master03'

for NODE in $MasterNodes; do 
    scp /usr/local/bin/{etcd*,kube*} $NODE:/usr/local/bin/; 
done
```
...
discovery-proxy:
discovery-srv:
initial-cluster: 'k8s-master01=https://192.168.2.4:2380,k8s-master02=https://192.168.2.5:2380,k8s-master03=https://192.168.2.6:2380'
...
election-timeout: 1000
quota-backend-bytes: 0
listen-peer-urls: 'https://192.168.2.5:2380'
listen-client-urls: 'https://192.168.2.5:2379,http://127.0.0.1:2379'
max-snapshots: 3
max-wals: 5
cors:
initial-advertise-peer-urls: 'https://192.168.2.5:2380'
advertise-client-urls: 'https://192.168.2.5:2379'
discovery:
discovery-fallback: 'proxy'
discovery-proxy:
discovery-srv:
...
election-timeout: 1000
quota-backend-bytes: 0
listen-peer-urls: 'https://192.168.2.6:2380'
listen-client-urls: 'https://192.168.2.6:2379,http://127.0.0.1:2379'
max-snapshots: 3
max-wals: 5
cors:
initial-advertise-peer-urls: 'https://192.168.2.6:2380'
advertise-client-urls: 'https://192.168.2.6:2379'
discovery:
discovery-fallback: 'proxy'
discovery-proxy:
discovery-srv:
...
backend k8s-master
  ...
  server k8s-master01    192.168.2.4:6443  check
  server k8s-master02    192.168.2.5:6443  check
  server k8s-master03    192.168.2.6:6443  check
EOF
```

#### 2.3 配置各master节点keepalived
...
#### 2.5 所有master节点启动haproxy和keepalived
...
#### 3.1 生成kube-apiserver systemd启动脚本
...
#### 3.2 所有master启动kube-apiserver并配置为开机启动
...
#### 4.1生成kube-controller-manager systemd启动脚本
...
#### 4.2启动kube-controller-manager并配置为开机启动
...
#### 5.1 生成kube-scheduler systemd启动脚本
...
```bash
kubectl label nodes k8s-master01 k8s-master02 k8s-master03 node-role.kubernetes.io/master=
```
...
```bash
kubectl taint node k8s-master01 k8s-master02 k8s-master03 node-role.kubernetes.io/master:NoSchedule
```
